...
If causal support is a good normative model for causal reasoning, we expect to see that judgments are closer to causal support when sample size is larger and when the effectiveness of the intervention is very large or very small. Another way of saying this is that error in judgments should be larger when causal support suggests ground truth probabilities closer to 0.5. An alternative interpretation of this hypothesis is that we expect causal support to predict how difficult the task is for users, which should manifest in the size of absolute errors.
